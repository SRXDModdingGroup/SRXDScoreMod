...
public class ScoreContainer {
    ...
    private int[] possibleNotePoints;
    private int[] possibleHoldTicks;
    private int[] possibleSpinTicks;
    private int[] possibleScratchTicks;
    ...
    public void Initialize(string trackId, int noteCount, int holdCount, int spinCount, int scratchCount) {
        Score = 0;
        Multiplier = Profile.MaxMultiplier;
        ...
        possibleNotePoints = new int[noteCount];
        possibleHoldTicks = new int[holdCount];
        possibleSpinTicks = new int[spinCount];
        possibleScratchTicks = new int[scratchCount];
    }

    public Accuracy HitNote(NoteType noteType, float timingOffset) {
        switch (noteType) {
            case NoteType.Match:
            ...
            case NoteType.SpinLeftStart:
                AddFlatScore(Profile.SpinStartValue);
    ...
    public void ClearNote(int noteIndex) {
        MaxScoreSoFar += possibleNotePoints[noteIndex];
        possibleNotePoints[noteIndex] = 0;
    }

    public void ClearHoldTick(int holdIndex) => ClearSustainedNoteTick(holdIndex, possibleHoldTicks);
        
    public void ClearSpinTick(int spinIndex) => ClearSustainedNoteTick(spinIndex, possibleSpinTicks);
        
    public void ClearScratchTick(int scratchIndex) => ClearSustainedNoteTick(scratchIndex, possibleScratchTicks);

    public void ClearAllHoldTicks(int holdIndex) => ClearAllSustainedNoteTicks(holdIndex, possibleHoldTicks);

    public void ClearAllSpinTicks(int spinIndex) => ClearAllSustainedNoteTicks(spinIndex, possibleSpinTicks);

    public void ClearAllScratchTicks(int scratchIndex) => ClearAllSustainedNoteTicks(scratchIndex, possibleScratchTicks);
    ...
    private void ClearSustainedNoteTick(int noteIndex, int[] possibleTicks) {
        ...
        MaxScoreSoFar += amount;
        possibleTicks[noteIndex] -= amount;
    }

    private void ClearAllSustainedNoteTicks(int noteIndex, int[] possibleTicks) {
        MaxScoreSoFar += possibleTicks[noteIndex];
        possibleTicks[noteIndex] = 0;
    }
